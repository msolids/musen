...
# Requires common_config.sh, copy_source_files.sh, make_musen_host.sh, musen.sh in the same directory to run.
#
# To select version of MUSEN to build, run the script as
# $ ./compile_on_host.sh [--target=cli] [--target=gui] [--target=matlab]
# or 
# $ ./compile_on_host.sh [-t=c] [-t=g] [-t=m]
# where 
...
# compile
echo "Compile MUSEN"
./make_musen_host.sh ${TARGETS_ARGS}
