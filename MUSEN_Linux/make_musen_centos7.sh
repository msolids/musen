...
# $ ./make_musen_centos7.sh [--target=cli] [--target=gui] [--target=matlab]
# or 
# $ ./make_musen_centos7.sh [-t=c] [-t=g] [-t=m]
# where 
...
# 
# Build requirements:
# gcc 9                - run './install_gcc_centos7.sh'      to install into default path
# cmake v3.17 or newer - run './install_cmake_centos7.sh'    to install into default path
# zlib v1.2.13         - run './install_zlib_centos7.sh'     to install into ${MUSEN_EXTERNAL_LIBS_PATH}/zlib
# protobuf v3.14.0     - run './install_protobuf_centos7.sh' to install into ${MUSEN_EXTERNAL_LIBS_PATH}/protobuf
# Qt v5.15.2           - run './install_qt_centos7.sh'       to install into ${MUSEN_EXTERNAL_LIBS_PATH}/qt
# CUDA v11.2           - run './install_cuda_centos7.sh'     to install into default path
...
cmake3 protobuf -S ${MUSEN_SRC_PATH} -B ${MUSEN_BUILD_PATH}
make protobuf --directory ${MUSEN_BUILD_PATH} --silent --jobs=8
cmake3 -S ${MUSEN_SRC_PATH} -B ${MUSEN_BUILD_PATH}
