...
void CModelPWPopovJKR::CalculatePW(double _time, double _timeStep, size_t _iWall, size_t _iPart, const SInteractProps& _interactProp, SCollision* _collision) const
{
	...
	const double adhForce = 3. / 2. * _interactProp.dEquivSurfaceTension * PI * partContactRadius;
	const double SAdh = std::pow(0.4626 * std::pow(_interactProp.dEquivSurfaceTension / _interactProp.dEquivYoungModulus, 2) * partContactRadius, 1. / 3.);
