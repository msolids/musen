...
in highp vec3 v_position;	// vertex coordinates
in highp vec3 v_normal;		// normal vector
...
void main()
{
	highp vec3 viewPos            = vec3(0.0, 0.0, 1.0);
	highp vec3 lightPos           = vec3(1.0, 1.0, 1.0);
	...
	o_color = vec4(ambient + diffuse + specular, 1.0) * v_color;
}
